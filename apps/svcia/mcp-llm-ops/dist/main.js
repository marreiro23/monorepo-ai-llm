...
server.registerTool('prompts_list', {
    ...
    description: 'Lista templates de prompt por GET /llm-ops/prompt-templates. Somente leitura.',
    inputSchema: {
    ...
    const payload = { llmOpsApiUrl, agentId: agentId ?? null, count: prompts.length, prompts };
    return toToolResult(payload);
});
server.registerTool('topic_flows_list', {
    title: 'List Topic Flows',
    description: 'Lista topic flows por GET /llm-ops/topic-flows. Somente leitura.',
    inputSchema: {
        agentId: z.string().uuid().optional(),
    ...
    const topicFlows = await llmOpsClient.listTopicFlows(agentId, topicDomain, limit);
    const payload = {
        llmOpsApiUrl,
...
server.registerTool('resources_catalog', {
    title: 'List Resource Catalog',
    description: 'Lista o catálogo de recursos consultáveis em modo read-only recommendation.',
    inputSchema: {}
}, async () => {
...
server.registerTool('ask_and_answer', {
    title: 'Ask And Answer',
    description: 'Executa consulta RAG/chat via POST /llm-ops/chat. Nao executa mutacao administrativa.',
    inputSchema: {
...
}, async ({ message, agentId, promptTemplateId, topicFlowId, topicFlowVersionId, invocationSource = 'api', sessionFingerprint }) => {
