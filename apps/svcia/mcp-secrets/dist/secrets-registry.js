const DEFAULT_SECRET_CATALOG = [
    ...
    },
    {
        ...
        description: 'Microsoft Entra certificate thumbprint for Graph authentication.',
        rotationSupported: true
    },
    {
    ...
    },
    {
        ...
        provider: 'process-env',
        requiredBy: ['apps/api', 'graph-auth'],
        description: 'Microsoft Graph client secret (alternative to certificate auth).',
        rotationSupported: true
    },
    {
    ...
    },
    {
        ...
        description: 'PostgreSQL password for the primary (public schema) connection.',
        rotationSupported: true
    },
    {
        ...
        description: 'PostgreSQL superuser password used by the Docker postgres service.',
        rotationSupported: true
    },
    {
...
export class SecretsRegistry {
    catalog;
    env;
    constructor(env = process.env, catalog = DEFAULT_SECRET_CATALOG) {
        this.env = env;
        this.catalog = [...catalog].sort((left, right) => left.logicalKey.localeCompare(right.logicalKey));
    }
    listSecrets(options = {}) {
    ...
    prepareOperation(input) {
        ...
        const correlationId = input.correlationId?.trim() || this.createCorrelationId(input.action, catalogEntry.logicalKey);
    ...
    findOrCreateEntry(input) {
        const logicalKey = input.logicalKey.trim();
        const existing = this.catalog.find((entry) => entry.logicalKey === logicalKey);
        if (existing) {
            return existing;
        }
        return {
            ...
            requiredBy: [],
            description: 'Dry-run metadata for a logical key not yet present in the catalog.',
            rotationSupported: true
        };
    }
    toMetadata(entry) {
        return {
            ...entry,
            configured: Object.prototype.hasOwnProperty.call(this.env, entry.targetName),
            valueStatus: 'redacted',
            valuePreview: '[redacted]'
        };
    }
    stepsFor(action, entry) {
        const base = [
    ...
    toEnvName(logicalKey) {
        return logicalKey.replace(/[^A-Za-z0-9]+/g, '_').replace(/^_+|_+$/g, '').toUpperCase();
    }
    createCorrelationId(action, logicalKey) {
