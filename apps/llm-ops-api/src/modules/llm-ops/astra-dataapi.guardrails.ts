...
export function buildLexicalField(...parts: Array<string | null | undefined>): string {
  return normalizeIndexedString(parts.filter((item): item is string => Boolean(item)).join(' '));
}
...
export function assertAstraDocumentShape(document: unknown, label: string): void {
  ...
  if (inspection.maxObjectProperties > ASTRA_DATA_API_LIMITS.maxObjectProperties) {
    throw new Error(
      `${label} exceeds Astra object property limit (${inspection.maxObjectProperties} > ${ASTRA_DATA_API_LIMITS.maxObjectProperties})`
    );
  }

  if (inspection.totalProperties > ASTRA_DATA_API_LIMITS.maxDocumentProperties) {
    throw new Error(
...
export function summarizeAstraError(error: unknown): string {
  if (error instanceof Error) {
    const rawResponse = (error as Error & { rawResponse?: { errors?: Array<{ message?: string; title?: string }> } }).rawResponse;
    const apiErrors = rawResponse?.errors?.map((item) => item.message || item.title).filter(Boolean);
...
function inspectJson(value: unknown): JsonInspection {
  ...
  const visit = (current: unknown, depth: number, path: string[]): void => {
    ...
    initial.maxObjectProperties = Math.max(initial.maxObjectProperties, entries.length);
    initial.maxDepth = Math.max(initial.maxDepth, depth);

    for (const [key, nestedValue] of entries) {
      const nextPath = [...path, key];
      initial.totalProperties += 1;
      initial.maxPathLength = Math.max(initial.maxPathLength, nextPath.join('.').length);
