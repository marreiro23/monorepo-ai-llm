...
export const RESOURCE_CATALOG: LlmOpsResourceCatalogItemContract[] = [
  ...
  },
  {
    ...
    readableEndpoints: ['/graph/config', '/graph/auth/status', '/graph/sites', '/graph/users', '/graph/groups'],
    writeEndpoints: ['/graph/groups (POST)', '/graph/provisioning/team-site (POST)'],
    llmAccess: RESOURCE_EXECUTION_MODE
  },
  {
    domain: 'sharepoint',
    description: 'Operações de bibliotecas, itens e permissões SharePoint.',
    readableEndpoints: ['/sharepoint/drives/:driveId/items', '/sharepoint/sites/:siteId/lists/:listId/items'],
  ...
  },
  {
    ...
    readableEndpoints: ['/sync/jobs', '/sync/sites', '/sync/users', '/sync/groups', '/sync/teams'],
  ...
  },
  {
    ...
    readableEndpoints: ['/governance', '/governance/permissions/matrix', '/governance/permissions/validation'],
    writeEndpoints: [],
    llmAccess: RESOURCE_EXECUTION_MODE
  },
  {
  ...
  },
  {
    ...
    readableEndpoints: ['/llm-ops/agents', '/llm-ops/prompt-templates', '/llm-ops/topic-flows'],
...
const DOMAIN_KEYWORDS: Array<{ domain: LlmOpsResourceDomainContract; keywords: string[] }> = [
  { domain: 'users', keywords: ['user', 'users', 'usuario', 'usuarios', 'membro', 'membros'] },
  { domain: 'graph', keywords: ['graph', 'entra', 'azure ad', 'group', 'groups', 'team', 'teams'] },
  { domain: 'sharepoint', keywords: ['sharepoint', 'drive', 'library', 'lista', 'list item', 'documento'] },
  { domain: 'sync', keywords: ['sync', 'sincron', 'job', 'jobs', 'replicacao'] },
  { domain: 'governance', keywords: ['governance', 'permiss', 'compliance', 'matriz', 'openapi'] },
  { domain: 'audit', keywords: ['audit', 'auditoria', 'log', 'trilha', 'evidencia'] },
  { domain: 'llm-ops', keywords: ['llm', 'rag', 'prompt', 'topic flow', 'ask and answer', 'langflow', 'astra'] }
];
  ...
  for (const candidate of DOMAIN_KEYWORDS) {
    if (candidate.keywords.some((keyword) => normalized.includes(keyword))) {
      return {
        ...
        resource: candidate.keywords.find((keyword) => normalized.includes(keyword)) ?? candidate.domain
      };
    }
  }
